...
<p align="center">
  A local-first, self-improving AI agent: a voice/vision companion and self-editing coding harness, powered by <a href="https://venice.ai">Venice AI</a> or <a href="https://surplusintelligence.ai">Surplus Intelligence</a>, with a control plane (Gondola Lab) that evaluates and promotes changes to how it works.
</p>

<p align="center">
  <img alt="License: MIT" src="https://img.shields.io/badge/license-MIT-blue.svg" />
  <img alt="Node 20+" src="https://img.shields.io/badge/node-20%2B-3c873a.svg" />
  <img alt="Powered by Venice or Surplus" src="https://img.shields.io/badge/powered%20by-Venice%20%7C%20Surplus-8d70c8.svg" />
</p>

Gondola pairs two front ends over one agent core. The terminal harness is a coding agent that reads, writes, edits, and runs code in its working directory, so pointed at its own repository it can edit itself. The web companion adds a voice and vision interface with an animated presence, a media tray, a workspace for agents and memory, and scheduled automations. Every model call goes through a configured inference provider (Venice AI or Surplus Intelligence), and your data stays on your machine.
...
Requirements: Docker (optional), Node.js 20+ and an inference API key (Venice AI or Surplus Intelligence).
...
On first run, Gondola shows a guided setup that verifies your provider key and turns on capabilities. Developers can skip it by adding a key to `.env.local` as above.
...
| Model selection | Choose any model from the active provider per conversation, with automatic fallback when one is unavailable. Reasoning models can stream a collapsible thinking trace. |
...
| API X-ray | A live trace of every inference call with latency, tokens, and the exact per-request cost from model list pricing. |
...
| Self-modification | Changes its own chat model on request, lists the live model catalog from the active provider, authors new abilities (pending your approval), and can flag recurring problems to the Lab for a reviewed fix. |
...
flowchart TB
  ...
  Agent --> Provider["Inference provider (Venice AI or Surplus Intelligence): chat, vision, speech, image, video, music, embeddings"]
...
- Web companion (`src/app/`): the browser handles UI, webcam/mic, and audio; local Next.js API routes keep the provider key private and stream agent events as newline-delimited JSON.
...
- Both share `src/lib/`: the inference client, memory, model and stream setup, skills, MCP, sub-agents, search, and compaction.
- Pi Agent Core orchestrates the loop, tools, memory, and compaction. It does not replace the inference provider; every capability goes through the configured provider (Venice AI or Surplus Intelligence).
