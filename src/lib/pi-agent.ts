...
- Budget is a soft guideline, not a wall. The only real spend gate is the one-time confirmation for a single costly item. Confirm that once; the moment the owner says to proceed, says the budget is unlimited, or tells you to stop asking about budget, set confirmed=true, do it, and do not raise the budget again this session. Never say you "cannot" generate because a declared or runtime budget is exceeded - that figure is informational and the owner just overrode it. Ask about cost at most once per item, never repeatedly.
...
function createTools(runtime: RuntimeContext): AgentTool[] {
  ...
  const imageTool: AgentTool = {
    ...
    description: "Generate an image with the selected Venice image model. It renders inline in the chat. Optionally set width and height in pixels (up to 1280 per side) to make ANY shape the task needs - there is no fixed menu: e.g. 1280x320 for a 4:1 banner, 720x1280 for a vertical 9:16 reel/story, 1280x720 for 16:9, 1024x1024 square (the default). You fully control the exact pixels; never say you can only make squares or cannot set dimensions.",
    parameters: Type.Object({
      prompt: Type.String({ minLength: 3, maxLength: 1500 }),
      title: Type.Optional(Type.String({ maxLength: 80 })),
      width: Type.Optional(Type.Integer({ minimum: 256, maximum: 1280 })),
      height: Type.Optional(Type.Integer({ minimum: 256, maximum: 1280 })),
    }),
    async execute(_toolCallId, params, signal) {
  ...
  const videoTool: AgentTool = {
    ...
    parameters: Type.Object({
      ...
      aspect_ratio: Type.Optional(Type.String({ maxLength: 8 })),
